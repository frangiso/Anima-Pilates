          ...
          return (
            <div key={hora} className="card" style={{
              ...
              {horaSelec === hora && ocupados > 0 && (
                <div style={{ marginTop: 14, borderTop: '1px solid #e0f0e6', paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {alumnas.map(a => (
                    <div key={a.id} style={{
                      display: 'flex',
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        {a.asistio === undefined ? (
                          <>
                            <button className="btn btn-secondary" style={{ padding: '6px 14px', minHeight: 34, fontSize: '0.85rem' }}
                              onClick={e => { e.stopPropagation(); marcarAsistencia(a.id, true) }}>
                              ✓ Asistió
                            </button>
                            <button className="btn btn-ghost" style={{ padding: '6px 14px', minHeight: 34, fontSize: '0.85rem' }}
                              onClick={e => { e.stopPropagation(); marcarAsistencia(a.id, false) }}>
                              ✗ Faltó
                            </button>
                          </>
                        ) : (
